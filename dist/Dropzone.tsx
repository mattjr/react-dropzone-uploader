  ...
  uploadFile = async (fileWithMeta: IFileWithMeta) => {
    ...
    for (const field of Object.keys(fields)) {
      ...
      if (value !== undefined) { // Check if the value is not undefined
        ...
        // Handle the undefined case (skip, default value, etc.)
        // Example: formData.append(field, '');
      }
    }
    xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest')
    for (const header of Object.keys(headers)) {
      const value = headers[header];
      if (value !== undefined) { // Ensure the value is not undefined
        xhr.setRequestHeader(header, value);
      } else {
        // Optionally handle the undefined case (skip setting the header, use a default value, etc.)
        // For skipping, just continue to the next iteration.
      }
    }
